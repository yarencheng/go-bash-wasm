---
description: Workflow for documenting technical deviations, workarounds, and sandbox limitations in docs/bash/functional_gap.md and docs/coreutils/functional_gap.md
---

// turbo-all

This workflow guides the AI agent on how to capture and document technical rationales for features that cannot be implemented 1:1 with upstream GNU/Bash behavior, ensuring all deviations are justified and tracked in `docs/bash/functional_gap.md` or `docs/coreutils/functional_gap.md`.
...
Before starting, review `docs/bash/tasks.md` or `docs/coreutils/tasks.md` for the target command.
...
Update the appropriate `docs/*/functional_gap.md` file using the established status codes:
...
Ensure that every entry in the gap maps has a corresponding reference (even if just a placeholder) in the tasks.md files to maintain a single source of truth for progress.
