...
func (d *Df) Run(ctx context.Context, env *commands.Environment, args []string) int {
	...
	_ = flags.Bool("sync", false, "invoke sync before getting usage info (ignored)")
	_ = flags.Bool("no-sync", false, "do not invoke sync before getting usage info (ignored)")
	_ = flags.String("output", "", "use the output format defined by FIELD_LIST (ignored)")
